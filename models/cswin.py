...
@register_model
def CSWin_64_12211_tiny_224(pretrained, pretrained_cfg):
    depths = getattr(pretrained_cfg,"depths",[1,2,21,1])
    num_heads = getattr(pretrained_cfg,"num_heads",[2,4,8,16])
    split_size = getattr(pretrained_cfg,"split_size",[1,2,7,7])
    model = CSWin(patch_size=4, embed_dim=64, depth=depths,
        split_size=split_size, num_heads=num_heads, mlp_ratio=4.)
    if pretrained and os.path.exists(pretrained_cfg.encoder_pretrained_url):
        checkpoint = torch.load(pretrained_cfg.encoder_pretrained_url,
            map_location="cpu")
        new_checkpoint = {}
        model_keys = model.state_dict().keys()
        for k in checkpoint["state_dict_ema"].keys():
            if k in model_keys and model.state_dict()[k].shape == checkpoint["state_dict_ema"][k].shape:
                new_checkpoint[k] = checkpoint["state_dict_ema"][k]
            else:
                print("key",k)
...
@register_model
def CSWin_64_24322_small_224(pretrained, pretrained_cfg):
    depths = getattr(pretrained_cfg,"depths",[2,4,32,2])
    num_heads = getattr(pretrained_cfg,"num_heads",[2,4,8,16])
    split_size = getattr(pretrained_cfg,"split_size",[1,2,7,7])
    model = CSWin(patch_size=4, embed_dim=64, depth=depths,
        split_size=split_size, num_heads=num_heads, mlp_ratio=4.)
...
if __name__=="__main__":
    ...
    class pretrain_cfg:
        ...
        num_heads = [2,4,8,16]
        split_size = [1,2,7,7]
    model = create_model("CSWin_64_12211_tiny_224",pretrain_cfg)
    image = torch.randn((3,3,64,336))
    output = model(image)
    print(output.shape)